...
class Pattern_Extract:
    ...
    def Decomposition(self,img,lr=0.01,rou_A = 0.01):
        img = torch.tensor(img, device=self.device, dtype=self.dtype, requires_grad=True)
        cur_A = torch.zeros(img.shape).to(self.device)
        cur_A = nn.Parameter(cur_A, requires_grad=True)
        cur_E = torch.zeros(img.shape).to(self.device)
        # cur_E = nn.Parameter(cur_E, requires_grad=True)
        cur_optimizer_A = torch.optim.Adam([cur_A], lr=lr)
        # cur_optimizer_E = torch.optim.Adam([cur_E], lr=self.learning_rate_A_E)
        ...
        for i in range(self.max_iteration):
            # loss
            loss1 = (torch.mm(RA2, torch.mm(RA1, img - cur_A - cur_E).transpose(0, 1)) - (
                    img - cur_A - cur_E).transpose(0, 1)).pow(2).sum()
            loss2 = rou_A * torch.norm(cur_A, 1)
            loss_A = loss1 + loss2
            cur_optimizer_A.zero_grad()
            loss_A.backward()
            cur_optimizer_A.step()

        # for i in range(self.max_iteration):
        #     # loss
        #     loss1 = (torch.mm(RA2, torch.mm(RA1, img - cur_A - cur_E).transpose(0, 1)) - (
        #             img - cur_A - cur_E).transpose(0, 1)).pow(2).sum()
        #     loss2 = self.rou_l2_E * cur_E.pow(2).sum()
        #     loss_E = loss1 + loss2
        #     cur_optimizer_E.zero_grad()
        #     loss_E.backward()
        #     cur_optimizer_E.step()

        Y_A_E = (img - cur_A - cur_E).cpu().detach().numpy()
        del cur_A,cur_E,img
        del cur_optimizer_A
        # del cur_optimizer_E
        return Y_A_E
